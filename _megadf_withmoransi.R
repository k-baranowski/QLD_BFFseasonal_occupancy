...
#all_hev2<- read.csv("all_hev2_megadf_everything2_20231205.csv", stringsAsFactors = F)
...
alldiet2<- alldiet[c(1:4)]
colnames(alldiet2) = c("camp", "year", "morI_alldiet", "pval_alldiet")

allwint<- read.csv("allwintdietuprr8m_moransi_stylB_results_20231121.csv", stringsAsFactors = F)
allwint2<- allwint[c(1:4)]
colnames(allwint2) = c("camp", "year", "morI_allwint", "pval_allwint")

atypcl<- read.csv("atypclwintuppr8m_moransi_stylB_fixresults_20231205.csv", stringsAsFactors = F)
atypcl<- subset(atypcl, year != 2000)
atypcl2<- atypcl[c(1:4)]
colnames(atypcl2) = c("camp", "year", "morI_atypclwint", "pval_atypclwint")

typcl<- read.csv("typclwintuppr8m_moransi_stylB_results_20231205.csv", stringsAsFactors = F)
typcl<- subset(typcl, year != 2000)
typcl2<- typcl[c(1:4)]
colnames(typcl2) = c("camp", "year", "morI_typclwint", "pval_typclwint")

typs<-  merge(typcl2, atypcl2, by = c("camp", "year"), all.x = T)
alls<- merge(alldiet2, allwint2, by = c("camp", "year"))
allmorans<- merge(alls, typs, by = c("camp", "year"), all.x = T)


mega<- merge(all_hev2, allmorans, by = c("camp", "year"), all.x = T)


mega<- mega %>%
  dplyr::group_by(camp_name) %>%
  fill(morI_allwint,morI_alldiet, morI_atypclwint,morI_typclwint,  .direction = "downup") %>%
  dplyr::ungroup()



write.csv(mega, "mega_finaldf_spillclock_envbatsmorI_20231205.csv", row.names = F)
...
mega %>%
  ...
  geom_point(aes(x = camp_name , y =morI_typclwint ), color = "dodgerblue3") +
  geom_point(aes(x = camp_name , y =morI_atypclwint ), color = "turquoise") +
...
onezero %>%
  ...
  ggplot() +
 # geom_point(aes(x = prop_20kbuff_yrobs , y =morI_allwint, color = as.factor(bff.presence))) +
 # geom_point(aes(x = prop_20kbuff_yrobs  , y =morI_alldiet, color = as.factor(bff.presence) )) +
 geom_point(aes(x = prop_20kbuff_yrobs  , y =morI_typclwint,color = as.factor(bff.presence) )) +
 # geom_point(aes(x =prop_20kbuff_yrobs  , y =morI_atypclwint,color = as.factor(bff.presence) )) +
  labs(x ='Proportion of Winter Habitat in 20km Buffer  ', y ='Morans I value Typical Winter',  color = "BFF Presence") +
  # labs(x ='Time', y ='Mean Precipitation Anomaly in 20km Rooost Foraging Buffers', shape = "year", color = "Spillover in Roost Buffer") +
  scale_color_manual(values = c("grey57", "red"  )) +
  # ylim(-3,3) +
...
surveys %>%
  ...
  ggplot() +
   geom_point(aes(x = morI_atypclwint, y =morI_typclwint,  color= as.factor(spill20km))) +
...
surveys %>%
 ...
 # geom_boxplot(aes(x = month,  y =morI_atypclwint,color = as.factor(bff.presence)),notch = T, outlier.shape = NULL ) +
